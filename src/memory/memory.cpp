...
Byte Memory::read(Address address) {
    ...
    if (address < WRAM_BANK_01_START && address >= WRAM_BANK_00_START)    { return _wram_bank_00[address - WRAM_BANK_00_START]; }
...
void Memory::write(Address address, Byte data) {
    if (address < ROM_BANK_01_START)                                      { _rom_bank_00[address] = data; return; }
    if (address < VRAM_START)                                             { _rom_bank_01[address - ROM_BANK_01_START] = data; return; }
    if (address < ERAM_START)                                             { _vram[address - VRAM_START] = data; return; }
    if (address < WRAM_BANK_01_START && address >= WRAM_BANK_00_START)    { _wram_bank_00[address - WRAM_BANK_00_START] = data; return; }
    if (address < ECHO_START)                                             { _wram_bank_01[address - WRAM_BANK_01_START] = data; return; }
    if (address <= HRAM_START && address < IE_REGISTER)                   { _hram[address - HRAM_START] = data; return; }
